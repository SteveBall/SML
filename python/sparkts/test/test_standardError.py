...
def test_stdErr1(hive_context):
    ...
    inDf = hive_context.read.json(inData)
    print("Input Data")
    inDf.show()

    # Expected data
    expectedData = "../../../src/test/resources/sml/outputs/StandardErrorExpected.json"
    expectedDf = hive_context.read.json(expectedData).select("ref", "xColumn", "yColumn", "zColumn", "stdError")
    print("Expected Data")
    expectedDf.show()
