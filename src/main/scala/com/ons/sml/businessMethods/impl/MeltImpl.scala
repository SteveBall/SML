...
object MeltImpl {
  ...
  implicit class MeltMethodsImpl(df: DataFrame) extends BaseMethodsImpl(df : DataFrame) {
    ...
    /**
      ...
      * parameters are kept default)
      *
      *                                {{{ array<struct<variable : str, value: ... >> }}}
      *
      * This array is then added to the datframe under the column name '''_vars_and_vals'''
      *
      * A list of columns is then created, which holds only the '''id_vars''', the '''var_name''' and the '''value_name'''
      *
      * EG, '''var_name''' and '''value_name''' defaults.
      *
      *                 {{{  List(id_var[0], id_var[1], ...., id_var[n], _vars_and_vals['variable'] as 'variable', _
      *                 vars_and_vals['value'] as 'value') }}}
      *
      *
      * When the dataframe is selected with this list, it puts the variable part of the array as the variable column and
      * it puts the value part
      * of the array as the value column. Unpivoting the dataframe.
      *
